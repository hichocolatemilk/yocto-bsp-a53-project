...
### 1. 커널 최적화 및 BSP 설계 (BSP Engineering)
* **커널 다이어트**: `CONFIG_EXPERT` 모드를 통해 불필요한 드라이버(BT, Sound 등)를 제거하여 커널 사이즈를 약 **3.2MB 감량**하고 부팅 속도를 개선했습니다.
...
### 2. 보안 네트워크 어플리케이션 (Secure Networking)
* **독립적 개발 환경**: `populate_sdk`를 통해 툴체인을 추출하여 타겟 보드에 최적화된 **SDK 크로스 컴파일 환경**을 구축했습니다.
* **보안 통신 구현**: **OpenSSL(AES256-CBC, SHA256)** 라이브러리를 연동하여 데이터 암호화 및 무결성 검증 기능을 포함한 소켓 통신을 구현했습니다.
